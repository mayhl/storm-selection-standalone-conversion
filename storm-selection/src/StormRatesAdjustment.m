function [stmRate, lp_model, opts, d, SD] = StormRatesAdjustment(ind_storm, Resp, HC_target, opts, lp_model)
% Adjstment of storm rates in the subset to match a desired hazard curve by linear programming. 
% -- Inputs:
%  ind_storm:  a binary vector with 1 indicating storms included in the
%              subset.
...
%       .runtime_stormselection: (added after running 'StormSelection')
%                         runtime for the storm selection
%
...
%
%  lp_model: (OPTIONAL) a structure with model for linear programming corresponding to 
%            problem using ALL THE NODES IN THE ORIGINAL SUITE with following fields.
%            If it exists, storm rates are directly adjusted using only
%            both 'ind_storm' and 'lp_model'
...
%
% -- Outputs:
%  stmRate:    adjusted storm rates for selected storms.
%
...
%                         target rates (lp_model.tgtRates) for each node of interest
%
%            .weight_nodes: weights for nodes considered in objective
%                           function
%            .weight_rates: weights for surge rates considered in objective
%                           function
%
...
%            .runtime_rateadjustment: runtime for the storm rate adjustment
%
...
if ~isempty(lp_model)
    ...
    disp('Performing linear programming to adjust storm rates...')
    if strcmp(opts.gurobi, 'yes') % Using gurobi optimizer
        tic
        [d, SD, stmRate] = HC_match_LargeA_gurobi(ind_storm,lp_model,Resp,ind_obj,interp,HC_target_given); % if you has gurobi optimization        
        t = toc
        opts.runtime_rateadjustment = t;
    else
        tic
...
if sum(stmRate == 0) == 0
    fprintf('There is no storms with zero rates. \n');
else
    fprintf('WARNING: There are %d storms with zero rates. \n', sum(stmRate == 0));
    fprintf('WARNING: It is expected to take %.2f hours to complete Final Adjustment. \n', sum(stmRate == 0)*(Ns - sum(ind_storm))*opts.runtime_rateadjustment/3600)
    if opts.nselect > 0
        if isfield(opts, 'runtime_stormselection')
            if sum(stmRate == 0)*(Ns - sum(ind_storm))*opts.runtime_rateadjustment > opts.runtime_stormselection
